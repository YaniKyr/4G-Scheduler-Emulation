...
class BaseStation:
    def __init__(self, num_users: int, total_rbs: int):
        ...
        self.users: List[User] = [User(i,  self.generate_traffic_type(), self.generate_priority_level()) for i in range(num_users)]
    ...
    def generate_traffic_type(self) -> str:
        ...
        probabilities = [0.10, 0.15, 0.75]  # 75% voice call, 15% web browsing, 10% video streaming
    ...
    def print_results(self):
        ...
        headers = ["ID", "Traffic Type",  "Throughput", "Allocated RBs", "Initial Rac"]
        data = [[user.id, user.traffic_type, f"{user.throughput:.2f}", user.allocated_rbs, f"{user.InitRac:.2f}"] for user in self.users]
